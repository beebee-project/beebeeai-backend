...
function isActualRealShadowRecord(record) {
  ...
  return Boolean(
    ...
    (record.source === "REAL_SHADOW_TRAFFIC" ||
      payload.source === "REAL_SHADOW_TRAFFIC") &&
...
function privacyViolation(record) {
  return walk(
    record?.payload || {},
    (key, value) => /included$/i.test(key) && value === true,
  );
}
...
function summarizeCase(caseId, records) {
  ...
  const downloads = lifecycle.filter(
    (record) => lifecycleEvent(record) === "DOWNLOAD",
  );
  const deletes = lifecycle.filter(
    (record) => lifecycleEvent(record) === "DELETE",
  );
  const identities = new Set(executions.map(executionIdentity).filter(Boolean));
  const builderMinimumReady =
    executions.length >= BUILDER_MIN_EXECUTIONS_PER_CASE;
  const protocolReady =
...
function evaluateRealShadowObservationCollection({
  ...
  const caseSummaries = cases.map((item) =>
    summarizeCase(item.caseId, filtered),
  );
  const executionCount = filtered.filter(
    (record) => record.kind === "EXECUTION",
  ).length;
  const lifecycleCount = filtered.filter(
    (record) => record.kind === "LIFECYCLE",
  ).length;
  ...
  if (!limitedRuntime.ready) {
    errors.push(
      ...(limitedRuntime.errors || [limitedRuntime.reason]).map(
  ...
  const recordIds = filtered
    .map((record) => text(record.recordId, 128))
    .filter(Boolean)
    .sort();
...
function buildObservationCollectionSummary(
  result,
  { finalizedAt = new Date().toISOString() } = {},
) {
  if (!result || result.ready !== true) {
    const error = new Error(
      result?.reason || "REAL_SHADOW_OBSERVATION_COLLECTION_NOT_READY",
    );
  ...
  return Object.freeze({
    version:
      "query_candidate_planner_real_shadow_observation_collection_summary_v1",
    phase: "15.3-B",
    patch: "15.3.2-E",
