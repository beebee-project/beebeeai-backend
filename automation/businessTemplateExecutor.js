...
const TEMPLATE_REGISTRY_MODULE_PATH =
  require.resolve("./businessTemplates/templateRegistry");
const { getBusinessTemplateRegistryItem } = require(
  TEMPLATE_REGISTRY_MODULE_PATH,
);
...
const INVENTORY_SNAPSHOT_SUM_LABEL = "행별 재고 스냅샷 합계";
...
function buildRegistryObservation({
  ...
  return {
    ...
    businessTemplateRegistryModulePath: modulePathForObservation(
      TEMPLATE_REGISTRY_MODULE_PATH,
    ),
    businessTemplateDefinitionFound: definitionExists(definition),
    businessTemplateHasCustomExecutor: custom,
    businessTemplateRegisteredExecutorName: executorName(executor),
    businessTemplateImplementationLevel: String(
      definition?.implementationLevel || "",
    ),
    businessTemplateExecutionPath: custom
...
function executeRegisteredTemplate({
  ...
  try {
    registryItem = getBusinessTemplateRegistryItem(templateId) || {};
  } catch (error) {
    return {
      ok: false,
      registryItem: {},
      registryObservation: {
        ...
        businessTemplateRegistryError: error?.message || String(error),
      },
      errorResult: {
        ok: false,
  ...
  const definition =
    registryItem.definition && typeof registryItem.definition === "object"
  ...
  try {
    ...
    return {
      ...
      registryObservation: {
        ...registryObservation,
        businessTemplateRegistryExecutorError: error?.message || String(error),
      },
      errorResult: {
        ok: false,
  ...
  if (rawOutput && typeof rawOutput.then === "function") {
    ...
    return {
      ...
      errorResult: {
        ...
        message: "업무 템플릿 executor는 동기 결과를 반환해야 합니다.",
      },
    };
  }

  const normalized = normalizeRegistryExecutorOutput(rawOutput);
  const registryObservation = buildRegistryObservation({
    registryItem,
    executor,
...
function renderedMetricIdsFromSections(sections = []) {
  return uniqueStrings(
    (Array.isArray(sections) ? sections : []).flatMap(collectSectionMetricIds),
  );
}
...
function contractCoverageSupported(coverage = {}) {
  ...
  return Boolean(
    ...
    ((Array.isArray(coverage.sections) && coverage.sections.length > 0) ||
...
function buildContractCoverageObservation({
  ...
  return {
    ...
    contractSummaryCoverageStatus: String(
      error ? "EXECUTION_ERROR" : coverage?.status || "UNSUPPORTED",
    ),
    contractSummaryRecipeVersion: String(coverage?.version || ""),
    contractSummaryCatalogVersion: String(
      coverage?.contractCatalogVersion || "",
    ),
    contractSummaryContractId: String(coverage?.contractId || ""),
    contractSummarySelectedTableId: String(coverage?.selectedTableId || ""),
    contractSummarySelectedSheetName: String(coverage?.selectedSheetName || ""),
    contractSummarySectionCount: Number(sectionCount || 0),
    contractSummaryExpectedMetricIdCount: uniqueStrings(
      coverage?.expectedMetricIds,
    ).length,
    contractSummaryRenderedMetricIdCount: uniqueStrings(
      coverage?.renderedMetricIds,
    ).length,
    contractSummaryInactiveMetricIdCount: uniqueStrings(
      coverage?.inactiveMetricIds,
    ).length,
    contractSummaryErrorMetricIdCount: uniqueStrings(coverage?.errorMetricIds)
      .length,
    contractSummaryCoverageSupported: contractCoverageSupported(coverage),
    contractSummaryCoverageError: error ? error?.message || String(error) : "",
  };
}
...
function executeContractCoverage({
  ...
  try {
    ...
    return {
      coverage,
      sections,
      observation: buildContractCoverageObservation({
        coverage,
...
function mergeContractAndSemanticCoverage({
  ...
  const contract = coverageWithoutSections(contractCoverage);
  const semantic = coverageWithoutSections(semanticCoverage);
  const contractSupported = contractCoverageSupported(contractCoverage);
  ...
  const inactiveMetricIds = uniqueStrings(contract.inactiveMetricIds);
  const errorMetricIds = uniqueStrings(contract.errorMetricIds);

  const primary = contractSupported ? contract : semantic;

  return {
    ...primary,
    version: String(primary.version || ""),
    contractCatalogVersion: String(primary.contractCatalogVersion || ""),
    expectedMetricIds,
    renderedMetricIds,
    inactiveMetricIds,
    errorMetricIds,
    coverageSources: uniqueStrings([
      contractSupported ? "contract_driven_summary" : "",
      Object.keys(semantic).length ? "semantic_output_planner" : "",
    ]),
    contractCoverage: Object.keys(contract).length ? contract : undefined,
    semanticCoverage: Object.keys(semantic).length ? semantic : undefined,
    combinedCoverageVersion: "contract_semantic_coverage_union_v1",
  };
}
...
function canonicalizeHashValue(value) {
  ...
  if (typeof value === "object") {
    return Object.keys(value)
      ...
      .reduce((result, key) => {
        result[key] = canonicalizeHashValue(value[key]);
        return result;
      }, {});
  }
...
function resultRowsForContentHash(section = {}) {
  const rows = section?.result?.rows;
  return Array.isArray(rows) ? rows : [];
}

function sectionContentPayload(section = {}) {
  const result = section.result || {};

  return {
    version: BUSINESS_SECTION_CONTENT_HASH_VERSION,
    resultType: normalizeIdentityText(result.resultType || ""),
    operation: normalizeIdentityText(
      result.operation || result.recipeType || "",
    ),
    metricHeader: normalizeIdentityText(result.metric?.header || ""),
    groupHeader: normalizeIdentityText(result.groupBy?.header || ""),
    rows: resultRowsForContentHash(section),
  };
}
...
function sectionContentHash(section = {}) {
  return crypto
    .createHash("sha256")
    .update(stableHashJson(sectionContentPayload(section)), "utf8")
    .digest("hex");
}

function sectionTitleIdentity(section = {}) {
  return normalizeIdentityText(section.title || section.sectionId || "");
}

function applyMetricIdsToSection(section = {}, metricIds = []) {
...
function mergeEquivalentSections(primary = {}, duplicate = {}) {
  return applyMetricIdsToSection(primary, [
    ...collectSectionMetricIds(primary),
    ...collectSectionMetricIds(duplicate),
  ]);
}

function dedupeSectionsByContentHash(sections = []) {
  const kept = [];
  const seen = new Map();
  const removed = [];
  ...
  for (const section of Array.isArray(sections) ? sections : []) {
    ...
    const titleKey = sectionTitleIdentity(section);
    const contentHash = sectionContentHash(section);
    const dedupeKey = `${titleKey}|${contentHash}`;
    ...
    const keptIndex = seen.get(dedupeKey);
    kept[keptIndex] = mergeEquivalentSections(kept[keptIndex], section);

    removed.push({
      sectionId: String(section.sectionId || ""),
      title: String(section.title || ""),
      contentHash,
      retainedSectionId: String(kept[keptIndex]?.sectionId || ""),
    });
  }
...
const GENERIC_SECTION_OUTPUT_KEYS = new Set([
  "작업",
  "지표",
  "값",
  "합계",
...
function firstResultRowKey(section = {}) {
  const firstRow = resultRowsForContentHash(section)[0];
  ...
  return (
    Object.keys(firstRow).find(
      (key) => !GENERIC_SECTION_OUTPUT_KEYS.has(normalizeIdentityText(key)),
    ) || ""
  );
}

function sectionDimensionHeader(section = {}) {
  const result = section.result || {};
...
function uniqueSectionTitle(requestedTitle = "", usedTitles = new Set()) {
  const base = String(requestedTitle || "분석 결과").trim() || "분석 결과";

  if (!usedTitles.has(normalizeIdentityText(base))) {
    return base;
  }
...
function disambiguateDuplicateSectionTitles(sections = []) {
  const list = Array.isArray(sections) ? sections : [];
  const titleCounts = new Map();

  list.forEach((section) => {
    const key = sectionTitleIdentity(section);
  ...
  const output = list.map((section, index) => {
    const originalTitle = String(
      section.title || section.sectionId || `section_${index + 1}`,
    ).trim();
    const identity = normalizeIdentityText(originalTitle);
    let nextTitle = originalTitle;

    if (usedTitles.has(identity) && (titleCounts.get(identity) || 0) > 1) {
      const dimension = sectionDimensionHeader(section);
      const proposed = dimension
        ? `${originalTitle} (${dimension} 기준)`
        : `${originalTitle} (추가 분석)`;

      nextTitle = uniqueSectionTitle(proposed, usedTitles);
      ...
      nextTitle = uniqueSectionTitle(originalTitle, usedTitles);
    }

    usedTitles.add(normalizeIdentityText(nextTitle));
...
function normalizeInventorySnapshotOverviewLabels(sections = []) {
  let adjustedCount = 0;
  const adjustedSectionIds = [];

  const output = (Array.isArray(sections) ? sections : []).map((section) => {
    const sectionType = normalizeIdentityText(
      section.sectionType || section.result?.resultType || "",
    );

    if (sectionType !== "inventory_flow_overview") {
      return section;
    }

    const rows = resultRowsForContentHash(section);
    let changed = false;
    ...
    const nextRows = rows.map((row) => {
      ...
      return {
        ...row,
        지표: INVENTORY_SNAPSHOT_SUM_LABEL,
      };
    });

    if (!changed) return section;

    adjustedSectionIds.push(String(section.sectionId || ""));
    ...
    return {
      ...
      result: {
        ...
        meta: {
          ...(section.result?.meta || {}),
          inventorySnapshotLabelVersion: BUSINESS_SECTION_FINALIZATION_VERSION,
          inventorySnapshotAggregation: "row_snapshot_sum",
          inventorySnapshotLabel: INVENTORY_SNAPSHOT_SUM_LABEL,
        },
      },
    };
  });
  ...
  return {
    sections: output,
    adjustedCount,
    adjustedSectionIds: uniqueStrings(adjustedSectionIds),
  };
}

function finalizeBusinessTemplateSections(sections = []) {
  const inventoryNormalized =
    normalizeInventorySnapshotOverviewLabels(sections);
  const deduped = dedupeSectionsByContentHash(inventoryNormalized.sections);
  const disambiguated = disambiguateDuplicateSectionTitles(deduped.sections);

  return {
    sections: disambiguated.sections,
    meta: {
      businessSectionFinalizationVersion: BUSINESS_SECTION_FINALIZATION_VERSION,
      inventorySnapshotLabelAdjustedCount: inventoryNormalized.adjustedCount,
      inventorySnapshotLabelAdjustedSectionIds:
        inventoryNormalized.adjustedSectionIds,
      contentHashDedupeApplied: deduped.removed.length > 0,
      deduplicatedSectionCount: deduped.removed.length,
      deduplicatedSections: deduped.removed,
      duplicateTitleDisambiguationApplied: disambiguated.renamed.length > 0,
      disambiguatedSectionCount: disambiguated.renamed.length,
      disambiguatedSections: disambiguated.renamed,
    },
  };
}

function finalizeBusinessTemplateResult(result = {}) {
  const finalized = finalizeBusinessTemplateSections(result.sections || []);
  ...
  return {
    ...
    executionMeta: {
      ...
      ...finalized.meta,
      finalizedSectionCount: finalized.sections.length,
    },
  };
}
...
function attachContractCoverage({
  ...
  const finalized = finalizeBusinessTemplateResult(result);
  const sections = Array.isArray(finalized.sections) ? finalized.sections : [];

  return {
    ...finalized,
    contractSummaryCoverage: mergeContractAndSemanticCoverage({
      contractCoverage,
      semanticCoverage: finalized.contractSummaryCoverage || {},
      sections,
    }),
    executionMeta: {
      ...(finalized.executionMeta || {}),
      ...contractObservation,
      combinedExpectedMetricIdCount: uniqueStrings([
        ...(contractCoverage.expectedMetricIds || []),
        ...(finalized.contractSummaryCoverage?.expectedMetricIds || []),
      ]).length,
    },
  };
}
  ...
  try {
    const augmented = augmentBusinessTemplateResult({
      ...
      context: {
        ...
        registryBridgeVersion: BUSINESS_TEMPLATE_REGISTRY_BRIDGE_VERSION,
        registryObservation,
      },
    });

    const semanticExpectedMetricIdCount = uniqueStrings(
      augmented.contractSummaryCoverage?.expectedMetricIds,
    ).length;
...
function executeBusinessTemplate({
  ...
  const templateId = String(templateCandidate.templateId || "").trim();
  ...
  if (!registered.ok) {
    return {
      ...(registered.errorResult || {
        ...
        message: "업무 템플릿 실행에 실패했습니다.",
      }),
      executionMeta: buildExecutorObservationMeta({
        templateCandidate,
        normalizedQueryTables,
        registryObservation: registered.registryObservation || {},
      }),
    };
  }

  const registrySections = registered.sections || [];
  ...
  const contractExecution = executeContractCoverage({
    normalizedQueryTables,
    templateId,
  });
  const contractSections = contractExecution.sections || [];
  const sections = [...registrySections, ...contractSections];

  const registryAndContractObservation = {
    ...(registered.registryObservation || {}),
    ...(contractExecution.observation || {}),
    registryBaseSectionCount: registrySections.length,
    contractBaseSectionCount: contractSections.length,
    preSemanticBaseSectionCount: sections.length,
  };

  const baseResult =
    registered.baseResult && typeof registered.baseResult === "object"
  ...
  const result = {
    ...baseResult,
    ok: true,
    resultType: baseResult.resultType || "businessTemplate",
    templateId: baseResult.templateId || templateId,
    title: baseResult.title || templateCandidate.title || templateId,
    description: baseResult.description || templateCandidate.description || "",
    sections,
    contractSummaryCoverage: coverageWithoutSections(
  ...
  return augmentResult({
    ...
    templateCandidate,
    registryObservation: registered.registryObservation || {},
    contractCoverage: contractExecution.coverage || {},
    contractObservation: contractExecution.observation || {},
  });
}
