...
const QUERY_CANDIDATE_PLANNER_INPUT_VERSION =
...
const QUERY_CANDIDATE_PLANNER_CACHE_VERSION =
  "query_candidate_planner_cache_v1";
...
const ALLOWED_OPERATIONS = Object.freeze({
  ...
  categorycount: Object.freeze({
    operation: "category_count",
    kinds: ["group"],
  }),
  groupsum: Object.freeze({
    operation: "group_sum",
    kinds: ["group", "measure"],
  }),
  groupavg: Object.freeze({
    operation: "group_avg",
    kinds: ["group", "measure"],
  }),
  ...
  timesum: Object.freeze({
    operation: "time_sum",
    kinds: ["period", "measure"],
  }),
  timeavg: Object.freeze({
    operation: "time_avg",
    kinds: ["period", "measure"],
  }),
    ...
    proposals: {
      ...
      items: {
        ...
        properties: {
          ...
          operation: {
            type: "string",
            enum: Object.values(ALLOWED_OPERATIONS).map(
              (item) => item.operation,
            ),
          },
          sourceTableIds: {
...
function operationCategory(operation = "") {
  ...
  if (["countrows", "categorycount", "timecount"].includes(value))
    return "OVERVIEW";
...
function isMeasureColumn(column = {}) {
  ...
  return (
    normalizeLoose(column.semanticType) === "measure" ||
    (normalizeLoose(column.dataType) === "number" &&
      normalizeText(column.metricFamily))
  );
}
  ...
  return asArray(semanticProfile.tables)
    ...
    .map((table) => ({
      ...
      columnCount: Math.max(
        0,
        Number(table.shape?.columnCount || asArray(table.columns).length),
      ),
...
function deriveSemanticOpportunities(tables = []) {
  ...
  for (const table of asArray(tables)) {
    ...
    if (groups.length >= 2 && measures.length)
      tableCategories.push("CROSS_TAB");
...
function unresolvedCandidateSummaries(
  candidateResolution = {},
  ...
  for (const item of asArray(candidateResolution.candidates)) {
    ...
    result.push({
      ...
      operation: normalizeText(
        item.checks?.operandBinding?.operation || item.recipeId || "",
      ),
      sourceTableIds: sortedUnique(
        item.checks?.sourceScope?.matchedRootTableIds,
      ),
      reasonCodes: sortedUnique(
        asArray(item.reasons).map((reason) => reason.code),
      ).slice(0, 8),
    });
  }
...
function buildPlannerInput({
  ...
  const coveredCategories = sortedUnique(
    ready.map((item) => item.operationCategory),
  );
  ...
  const input = {
    ...
    source: {
      caseId: normalizeText(
        semanticProfile.source?.caseId ||
          candidateResolution.source?.caseId ||
          "",
      ),
    ...
    classification: {
      primaryDomain: normalizeText(
        classification.primaryDomain ||
          resolvedSemanticProfile.primaryDomain ||
          "UNKNOWN",
      ),
      datasetIntent: normalizeText(
        classification.datasetIntent ||
          resolvedSemanticProfile.datasetIntent ||
          "UNKNOWN",
      ),
      confidence: round(
        clamp(
    ...
    deterministicCoverage: {
      ...
      recommendedCount: asArray(
        candidateRankingResolution.recommendedCandidateIds,
      ).length,
      coveredCategories,
...
function evaluateConditionalPlannerTrigger(input = {}) {
  ...
  const maxRowCount = Math.max(
    0,
    ...eligibleTables.map((table) => Number(table.rowCount || 0)),
  );
  ...
  const complexEnough =
    maxRowCount >= COMPLEX_ROW_THRESHOLD &&
    totalColumns >= 3 &&
    opportunityCount > 0;
  ...
  if (!opportunityCount) {
    return {
      ...
      reason:
        "지원 operation으로 변환할 의미 기회가 없어 Planner를 호출하지 않습니다.",
      metrics: {
        readyCount,
        maxRowCount,
        totalColumns,
  ...
  if (readyCount === 0) {
    return {
      ...
      reason:
        "분석 가능한 데이터에 READY 후보가 없어 조건부 Planner 복구가 필요합니다.",
      metrics: {
        readyCount,
        maxRowCount,
        totalColumns,
  ...
  if (!complexEnough) {
    return {
      ...
      reason:
        "소규모 데이터에 실행 가능한 후보가 이미 있어 추가 LLM 호출을 생략합니다.",
      metrics: {
        readyCount,
        maxRowCount,
        totalColumns,
  ...
  if (missingCount >= 2) {
    return {
      ...
      reason:
        "복합 데이터에서 READY 후보가 적고 의미 기회 범주가 충분히 덮이지 않았습니다.",
      metrics: {
        readyCount,
        maxRowCount,
        totalColumns,
  ...
  if (unresolvedCount > 0 && missingCount > 0) {
    return {
      ...
      reason:
        "미해결 후보와 의미 기회 공백이 함께 존재해 조건부 Planner 검토가 필요합니다.",
      metrics: {
        readyCount,
        maxRowCount,
        totalColumns,
...
function validateCandidatePlannerModelOutput(output = {}) {
  ...
  if (output.version !== QUERY_CANDIDATE_PLANNER_MODEL_OUTPUT_VERSION) {
    errors.push(
      modelValidationIssue(
        "MODEL_OUTPUT_VERSION_INVALID",
        "version",
        "모델 출력 version이 유효하지 않습니다.",
      ),
    );
  }
  ...
  if (!Array.isArray(output.proposals)) {
    errors.push(
      modelValidationIssue(
        "MODEL_PROPOSALS_NOT_ARRAY",
        "proposals",
        "proposals는 배열이어야 합니다.",
      ),
    );
  }
  ...
  if (proposals.length > MAX_PROPOSALS) {
    errors.push(
      modelValidationIssue(
        "MODEL_PROPOSALS_LIMIT_EXCEEDED",
        "proposals",
        "제안 개수 제한을 초과했습니다.",
      ),
    );
  ...
  if (output.decision === "NO_ADDITION" && proposals.length) {
    errors.push(
      modelValidationIssue(
        "NO_ADDITION_HAS_PROPOSALS",
        "proposals",
        "NO_ADDITION은 proposals가 비어 있어야 합니다.",
      ),
    );
  }
  if (output.decision === "PROPOSE" && !proposals.length) {
    errors.push(
      modelValidationIssue(
        "PROPOSE_HAS_NO_PROPOSALS",
        "proposals",
        "PROPOSE는 하나 이상의 제안이 필요합니다.",
      ),
    );
  }
  const keys = new Set();
  proposals.forEach((proposal, index) => {
    ...
    if (
      !Array.isArray(proposal.sourceTableIds) ||
      proposal.sourceTableIds.length !== 1
    ) {
      errors.push(
        modelValidationIssue(
          "PROPOSAL_SOURCE_COUNT_INVALID",
          `${path}.sourceTableIds`,
          "sourceTableIds는 정확히 한 개여야 합니다.",
        ),
      );
    }
    if (!Array.isArray(proposal.operandBindings)) {
      errors.push(
        modelValidationIssue(
          "PROPOSAL_OPERANDS_NOT_ARRAY",
          `${path}.operandBindings`,
          "operandBindings는 배열이어야 합니다.",
        ),
      );
    }
    if (normalizeText(proposal.outputType) !== "summarysheet") {
      errors.push(
        modelValidationIssue(
          ...
          `${path}.outputType`,
          "outputType은 summarysheet여야 합니다.",
        ),
      );
    }
    if (
      ...
      Number(proposal.confidence) > 1
    ) {
      errors.push(
        modelValidationIssue(
          "PROPOSAL_CONFIDENCE_INVALID",
          `${path}.confidence`,
          "confidence는 0~1이어야 합니다.",
        ),
      );
    }
    for (const code of asArray(proposal.evidenceCodes)) {
      if (!MODEL_EVIDENCE_CODES.includes(code)) {
        errors.push(
          modelValidationIssue(
            "PROPOSAL_EVIDENCE_CODE_INVALID",
            `${path}.evidenceCodes`,
            "evidence code가 유효하지 않습니다.",
          ),
        );
      }
    }
...
function normalizedBindingList(bindings = []) {
  return asArray(bindings)
    ...
    .sort((left, right) =>
      `${left.kind}|${left.columnId}`.localeCompare(
        `${right.kind}|${right.columnId}`,
        "ko",
      ),
    );
}
...
function validateAndNormalizeProposal(proposal = {}, index = 0, context = {}) {
  ...
  if (
    sourceTableIds.length !== 1 ||
    !context.maps.tables.has(sourceTableIds[0])
  ) {
  ...
  if (
    kindKeys.some(
      (kind) =>
        Number(actualCounts[kind] || 0) !== Number(expectedCounts[kind] || 0),
    )
  ) {
  ...
  if (
    context.existingSignatures.has(signature) ||
    context.acceptedSignatures.has(signature)
  ) {
  ...
  const item = {
    ...
    operationCategory: operationCategory(
      definition?.operation || proposal.operation,
    ),
    sourceTableIds,
    operandBindings: bindings,
    outputType: "summarysheet",
...
function buildPlannerResolutionBase({ input, trigger, source = {} } = {}) {
  return {
    ...
    source: {
      ...
      candidateResolutionSha256: normalizeText(
        input?.source?.candidateResolutionSha256 || "",
      ),
      candidateFeasibilityResolutionSha256: normalizeText(
      ...
      semanticProfileSha256: normalizeText(
        input?.source?.semanticProfileSha256 || "",
      ),
...
async function runConditionalCandidatePlanner({
  ...
  if (!provider || typeof provider.plan !== "function") {
    return finalizeResolution({
      ...
      summary:
        "조건부 Planner 호출 조건이 충족됐지만 provider가 설정되지 않았습니다.",
      usage: { ...normalizeUsage({}), estimatedCostUsd: 0 },
      counts: { proposed: 0, accepted: 0, rejected: 0 },
      acceptedCandidateIds: [],
      rejectedProposalKeys: [],
  ...
  if (!providerResult) {
    try {
      ...
      return finalizeResolution({
        ...
        summary:
          "LLM Candidate Planner 호출이 실패해 기존 결정론적 결과만 유지합니다.",
        usage: { ...normalizeUsage({}), estimatedCostUsd: 0 },
        counts: { proposed: 0, accepted: 0, rejected: 0 },
        acceptedCandidateIds: [],
        rejectedProposalKeys: [],
    ...
    return finalizeResolution({
      ...
      invocation: {
        ...
        reasoningEffort: normalizeText(
          providerResult.reasoningEffort || reasoningEffort,
        ),
      ...
      decision: "FAILED_SAFE",
      summary:
        "LLM Candidate Planner 출력이 strict contract를 통과하지 못해 폐기했습니다.",
      usage: {
  ...
  return finalizeResolution({
    ...
    integrity: {
      ...
      acceptedCandidateIdsUnique:
        accepted.length ===
        new Set(accepted.map((item) => item.candidateId)).size,
      allAcceptedReferencesKnown: accepted.every(
        (item) =>
          item.sourceTableIds.every((tableId) =>
            context.maps.tables.has(tableId),
          ) &&
          item.operandBindings.every((binding) =>
            context.maps.columns.has(binding.columnId),
          ),
      ),
    },
  });
...
function validateQueryCandidatePlannerResolution(document = {}) {
  ...
  if (document.version !== QUERY_CANDIDATE_PLANNER_RESOLUTION_VERSION) {
    errors.push(
      issue(
        "version",
        "INVALID_VERSION",
        "Planner resolution version이 유효하지 않습니다.",
      ),
    );
  }
  if (document.itemVersion !== QUERY_CANDIDATE_PLANNER_ITEM_VERSION) {
    errors.push(
      issue(
        ...
        "Planner item version이 유효하지 않습니다.",
      ),
    );
  }
  if (document.policy?.version !== QUERY_CANDIDATE_PLANNER_POLICY_VERSION) {
    errors.push(
      issue(
        ...
        "Planner policy version이 유효하지 않습니다.",
      ),
    );
  }
  ...
  proposals.forEach((proposal, index) => {
    if (!PROPOSAL_DISPOSITION.includes(proposal.disposition)) {
      errors.push(
        issue(
          `proposals[${index}].disposition`,
          "INVALID_PROPOSAL_DISPOSITION",
          "proposal disposition이 유효하지 않습니다.",
        ),
      );
    }
    if (proposal.readyStatusAssigned !== false) {
      errors.push(
        issue(
          `proposals[${index}].readyStatusAssigned`,
          "READY_ASSIGNED_TOO_EARLY",
          "Planner는 READY를 부여하면 안 됩니다.",
        ),
      );
    }
    ...
    if (proposal.plannerItemSha256 !== expected) {
      errors.push(
        issue(
          `proposals[${index}].plannerItemSha256`,
          "SHA_MISMATCH",
          "Planner item SHA가 일치하지 않습니다.",
        ),
      );
    }
  });
  ...
  const rejected = proposals.filter((item) => item.disposition === "REJECTED");
  if (
    Number(document.counts?.proposed || 0) !== proposals.length ||
    Number(document.counts?.accepted || 0) !== accepted.length ||
    Number(document.counts?.rejected || 0) !== rejected.length
  ) {
    errors.push(
      issue(
        "counts",
        "COUNT_MISMATCH",
        "Planner proposal count가 실제 항목과 다릅니다.",
      ),
    );
  }
  if (
    document.invocation?.status === "SKIPPED" &&
    Number(document.invocation.providerCallCount || 0) !== 0
  ) {
    errors.push(
      issue(
        "invocation.providerCallCount",
        "SKIPPED_PROVIDER_CALLED",
        "SKIPPED 상태에서는 provider 호출이 없어야 합니다.",
      ),
    );
  }
  if (
    document.policy?.sourceCandidatesAreNotRemovedOrMutated !== true ||
    document.integrity?.sourceCandidatesPreserved !== true
  ) {
    errors.push(
      issue(
  ...
  if (!/^[a-f0-9]{64}$/.test(document.plannerResolutionSha256 || "")) {
    errors.push(
      issue(
        "plannerResolutionSha256",
        "INVALID_SHA",
        "Planner resolution SHA가 유효하지 않습니다.",
      ),
    );
  } else {
    ...
    if (expected !== document.plannerResolutionSha256) {
      errors.push(
        issue(
          "plannerResolutionSha256",
          "SHA_MISMATCH",
          "Planner resolution SHA가 일치하지 않습니다.",
        ),
      );
    }
  }
...
function createMockCandidatePlannerProvider({
  output,
  usage = {},
  model = "mock-planner",
} = {}) {
