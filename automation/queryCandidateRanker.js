...
const QUERY_CANDIDATE_RANKING_ITEM_VERSION =
...
const RANKING_TIER = Object.freeze(["PRIMARY", "SECONDARY", "ADDITIONAL", "NOT_APPLICABLE"]);
...
function candidateEvidenceByColumnId(candidate = {}) {
  ...
  const evidence = [
    ...
    ...asArray(candidate.checks?.requiredRoles).flatMap((role) => asArray(role.matched)),
...
function operationCategory(operation = "") {
  ...
  if (["countrows", "categorycount", "timecount", "genericcountbygroup"].includes(value)) {
    return "OVERVIEW";
  }
  if (["groupsum", "groupavg", "groupsummary", "compositionratio"].includes(value)) {
  ...
  if (["singlesourcedashboard", "multisourceschemaunion"].includes(value)) return "STRUCTURAL";
...
function operationUtility(operation = "", executorMode = "") {
  ...
  if (["groupsummary", "compositionratio", "crosssum", "crosscount"].includes(value)) return 7;
  if (["groupsum", "groupavg", "timesum", "timeavg", "cumulativesum"].includes(value)) return 6;
  if (["topbottom"].includes(value)) return 5;
  return 4;
}

function priorRetrievalScore(candidate = {}) {
  const value = normalizeText(candidate.previousRetrievalResult || "").toUpperCase();
...
function executorScore(feasibility = {}) {
  const mode = normalizeText(feasibility.executionPlan?.executorMode || "").toUpperCase();
...
function contractEvidenceScore(candidate = {}, feasibility = {}) {
  const operandStatus = normalizeText(candidate.checks?.operandBinding?.status || "").toUpperCase();
...
function domainScore(candidate = {}) {
  const status = normalizeText(candidate.checks?.domainAlignment?.status || "").toUpperCase();
...
function metricScore(feasibility = {}) {
  const status = normalizeText(feasibility.checks?.metricContract?.status || "").toUpperCase();
...
function metricEvidenceText({
  ...
  for (const operand of asArray(candidate.checks?.operandBinding?.operands)) {
    ...
    if (!["measure", "amount", "revenue", "score", "rating", "satisfaction", "quantity"].includes(kind)) continue;
...
function metricAggregationClass({
  ...
  const text = metricEvidenceText({ candidate, measureColumnIds, semanticColumns });
  const scaleTokens = [
    "만족도", "만족", "평가점수", "추천점수", "평점", "점수", "척도",
    "satisfaction", "rating", "score", "nps", "likert", "percent", "percentage", "비율", "율",
  ];
  if (scaleTokens.some((token) => text.includes(normalizeLoose(token)))) return "SCALE";

  const columns = measureColumnIds.map((columnId) => semanticColumns.get(columnId) || {});
  const additiveByMetadata = columns.some((column) =>
    normalizeLoose(column.defaultAggregation) === "sum" ||
    ["currency", "amount", "count", "quantity", "volume", "area", "weight"].includes(
      normalizeLoose(column.unitSemantic),
    ),
  );
  const additiveTokens = [
    "금액", "매출", "예산", "비용", "지출", "집행", "수량", "판매량", "건수",
    "인원", "면적", "중량", "총액", "합계", "amount", "revenue", "sales", "budget",
    "expense", "cost", "quantity", "volume", "count",
  ];
  const sourceText = normalizeLoose(`${source.primaryDomain || ""} ${source.datasetIntent || ""}`);
  if (
    additiveByMetadata ||
    additiveTokens.some((token) => text.includes(normalizeLoose(token))) ||
    ["financ", "budget", "sales", "revenue", "expense", "cost"].some((token) => sourceText.includes(token))
  ) {
    return "ADDITIVE";
  }
  return "NEUTRAL";
...
function degenerateGroupPenalty({
  ...
  for (const columnId of sortedUnique(axisColumnIds)) {
    ...
    if (!column || !Number.isFinite(column.uniqueRatio) || column.rowCount < 8) continue;
...
function scoreComponents(candidate = {}, feasibility = {}, context = {}) {
  const executorMode = normalizeText(feasibility.executionPlan?.executorMode || "");
  ...
  const components = {
    ...
    metricAggregationAffinity: metricAggregationAffinity(operation, metricClass),
    ...
    resolutionEvidence: normalizedScoreContribution(candidate.resolutionScore, 5),
...
function bindingSignatures(feasibility = {}) {
  const bindings = [
    ...
    ...asArray(feasibility.executionPlan?.requiredRoleBindings).map((binding) => ({
      kind: normalizeLoose(binding.role || "unknown"),
      columnIds: sortedUnique(binding.columnIds),
    })),
  ];
  const groupKinds = new Set(["group", "dimension", "category", "status", "product", "organization"]);
  const periodKinds = new Set(["period", "date", "datetime"]);
  const measureKinds = new Set([
    "measure", "amount", "revenue", "score", "rating", "satisfaction", "quantity", "count",
  ]);
  const axis = sortedUnique(
    bindings
      .filter((binding) => groupKinds.has(binding.kind) || periodKinds.has(binding.kind))
...
function diversityPenalty(item = {}, ranked = []) {
  const sameOperation = ranked.filter((previous) => previous.operation === item.operation).length;
  const sameAxis = item.axisSignature
    ? ranked.filter((previous) => previous.axisSignature === item.axisSignature).length
    : 0;
  const sameMeasure = item.measureSignature
    ? ranked.filter((previous) => previous.measureSignature === item.measureSignature).length
    : 0;
  const components = {
  ...
  return {
    components,
    total: rounded(Object.values(components).reduce((sum, value) => sum + value, 0)),
  };
}
...
function compareEligible(a = {}, b = {}) {
  ...
  if (a.originalScore !== b.originalScore) return b.originalScore - a.originalScore;
...
function rankReadyCandidates(eligible = []) {
  ...
  while (remaining.length) {
    ...
    const index = remaining.findIndex((item) => item.candidateId === selected.candidateId);
...
function buildRankingItem({ candidate = {}, familyMember = {}, family = {}, feasibility = {}, ranked = null }) {
  const candidateId = normalizeText(candidate.candidateId || feasibility.candidateId || "");
  if (!ranked) {
    return {
      version: QUERY_CANDIDATE_RANKING_ITEM_VERSION,
      candidateId,
      familyId: normalizeText(familyMember.familyId || feasibility.familyId || ""),
      feasibilityStatus: normalizeText(feasibility.feasibilityStatus || "NOT_APPLICABLE"),
      ...
      operationCategory: operationCategory(feasibility.executionPlan?.operation || ""),
      ...
      provenance: {
        ...
        feasibilityItemSha256: normalizeText(feasibility.feasibilityItemSha256 || ""),
...
function buildQueryCandidateRankingResolution({
  ...
  const semanticColumns = semanticProfileColumnMap(deterministicSemanticProfile);
  const rankingContext = {
    semanticColumns,
    source: {
      primaryDomain: normalizeText(candidateResolution.source?.primaryDomain || ""),
      datasetIntent: normalizeText(candidateResolution.source?.datasetIntent || ""),
    },
  };

  const eligible = [];
  for (const feasibility of asArray(candidateFeasibilityResolution.candidates)) {
    if (normalizeText(feasibility.feasibilityStatus || "").toUpperCase() !== "READY") continue;
    ...
    const family = families.get(normalizeText(familyMember.familyId || feasibility.familyId || "")) || {};
    ...
    eligible.push({
      ...
      operation: normalizeLoose(feasibility.executionPlan?.operation || "unknownoperation"),
      operationCategory: operationCategory(feasibility.executionPlan?.operation || ""),
      sourceTableIds: sortedUnique(feasibility.executionPlan?.sourceTableIds),
  ...
  const candidates = asArray(candidateResolution.candidates).map((candidate) => {
    const candidateId = normalizeText(candidate.candidateId || "");
    const familyMember = familyMembers.get(candidateId) || {};
    const family = families.get(normalizeText(familyMember.familyId || "")) || {};
  ...
  const counts = {
    ...
    primary: rankedItems.filter((candidate) => candidate.rankingTier === "PRIMARY").length,
    secondary: rankedItems.filter((candidate) => candidate.rankingTier === "SECONDARY").length,
    additional: rankedItems.filter((candidate) => candidate.rankingTier === "ADDITIONAL").length,
  ...
  const document = {
    ...
    source: {
      caseId: normalizeText(candidateResolution.source?.caseId || candidateResolution.caseId || ""),
      ...
      candidateResolutionPolicyVersion: normalizeText(candidateResolution.policy?.version || ""),
      candidateResolutionSha256: normalizeText(candidateResolution.resolutionSha256 || ""),
      candidateFamilyResolutionVersion: normalizeText(candidateFamilyResolution.version || ""),
      candidateFamilyPolicyVersion: normalizeText(candidateFamilyResolution.policy?.version || ""),
      candidateFamilyResolutionSha256: normalizeText(candidateFamilyResolution.familyResolutionSha256 || ""),
      candidateFeasibilityResolutionVersion: normalizeText(candidateFeasibilityResolution.version || ""),
      candidateFeasibilityPolicyVersion: normalizeText(candidateFeasibilityResolution.policy?.version || ""),
      candidateFeasibilityResolutionSha256: normalizeText(candidateFeasibilityResolution.feasibilityResolutionSha256 || ""),
      deterministicSemanticProfileVersion: normalizeText(deterministicSemanticProfile.version || ""),
      deterministicSemanticProfileSha256: normalizeText(deterministicSemanticProfile.profileSha256 || ""),
      primaryDomain: normalizeText(candidateResolution.source?.primaryDomain || ""),
      datasetIntent: normalizeText(candidateResolution.source?.datasetIntent || ""),
    },
    integrity: {
      ...
      feasibilityCandidateCount: asArray(candidateFeasibilityResolution.candidates).length,
      readyFeasibilityCount: asArray(candidateFeasibilityResolution.candidates).filter(
        (candidate) => normalizeText(candidate.feasibilityStatus || "").toUpperCase() === "READY",
      ).length,
  ...
  document.rankingResolutionSha256 = sha256(stableStringify({
    ...document,
...
function validateQueryCandidateRankingResolution(document = {}) {
  ...
  const candidateIds = candidates.map((candidate) => normalizeText(candidate.candidateId || ""));
  const rankedIds = ranked.map((candidate) => normalizeText(candidate.candidateId || ""));
  const readyIds = candidates
    .filter((candidate) => normalizeText(candidate.feasibilityStatus || "").toUpperCase() === "READY")
  ...
  const recommended = asArray(document.recommendedCandidateIds).map(normalizeText);
  const expectedRecommended = rankedIds.slice(0, Number(document.policy?.recommendedLimit || RECOMMENDED_LIMIT));

  if (document.version !== QUERY_CANDIDATE_RANKING_RESOLUTION_VERSION) {
    errors.push(issue("version", "VERSION_MISMATCH", "ranking resolution version이 올바르지 않습니다."));
  }
  if (document.policy?.version !== QUERY_CANDIDATE_RANKING_POLICY_VERSION) {
    errors.push(issue("policy.version", "POLICY_VERSION_MISMATCH", "ranking policy version이 올바르지 않습니다."));
  }
  for (const [index, candidate] of candidates.entries()) {
    if (!RANKING_DISPOSITION.includes(candidate.rankingDisposition)) {
      errors.push(issue(`candidates[${index}].rankingDisposition`, "INVALID_DISPOSITION", "지원하지 않는 ranking disposition입니다."));
    }
    if (!RANKING_TIER.includes(candidate.rankingTier)) {
      errors.push(issue(`candidates[${index}].rankingTier`, "INVALID_TIER", "지원하지 않는 ranking tier입니다."));
    }
    if (candidate.rankingDisposition === "RANKED") {
      if (candidate.feasibilityStatus !== "READY") {
        errors.push(issue(`candidates[${index}]`, "NON_READY_CANDIDATE_RANKED", "READY가 아닌 후보가 순위화됐습니다."));
      }
      if (!Number.isInteger(candidate.rank) || candidate.rank < 1) {
        errors.push(issue(`candidates[${index}].rank`, "INVALID_RANK", "rank는 1 이상의 정수여야 합니다."));
      }
      if (!Number.isFinite(candidate.rankingScore) || candidate.rankingScore < 0 || candidate.rankingScore > 100) {
        errors.push(issue(`candidates[${index}].rankingScore`, "INVALID_SCORE", "rankingScore는 0~100 범위여야 합니다."));
      }
    } else if (candidate.rank != null || candidate.rankingScore != null) {
      errors.push(issue(`candidates[${index}]`, "NOT_APPLICABLE_HAS_RANK", "비대상 후보에는 rank와 score가 없어야 합니다."));
    }
  }
  ...
  const checks = {
    ...
    rankSequenceContiguous: JSON.stringify(actualRanks) === JSON.stringify(expectedRanks),
  ...
  for (const [key, passed] of Object.entries(checks)) {
    if (!passed) errors.push(issue(`integrity.${key}`, "INTEGRITY_CHECK_FAILED", `${key} 검사가 실패했습니다.`));
  }
  if (Number(document.counts?.ranked || 0) !== ranked.length) {
    errors.push(issue("counts.ranked", "COUNT_MISMATCH", "ranked count가 실제 항목 수와 다릅니다."));
  }
  if (Number(document.counts?.readyInput || 0) !== readyIds.length) {
    errors.push(issue("counts.readyInput", "COUNT_MISMATCH", "readyInput count가 실제 READY 수와 다릅니다."));
  }
  if (Number(document.counts?.ranked || 0) + Number(document.counts?.notApplicable || 0) !== candidates.length) {
    errors.push(issue("counts", "STATUS_PARTITION_INCOMPLETE", "RANKED/NOT_APPLICABLE 분할이 전체 후보를 덮지 않습니다."));
  }
