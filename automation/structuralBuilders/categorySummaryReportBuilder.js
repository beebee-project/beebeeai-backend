...
function buildCategorySummaryCandidates({ table, config = {} }) {
  ...
  for (const dim of config.dimensions || []) {
    const dimensionHeader =
      dim.header || findColumnHeader(table, dim.hints || []);
    ...
    if (metricHeader && dim.includeSummary !== false) {
      candidates.push(
        makeTemplateCandidate({
          ...
          sectionType:
            dim.summarySectionType || dim.sectionType || "category_summary",
          recipeType: "group_summary",
          title:
            dim.summaryTitle || `${dimensionHeader}별 ${metricHeader} 요약`,
          tableId,
          columns: {
    ...
    if (dim.includeCount) {
      candidates.push(
        makeTemplateCandidate({
          sectionId:
            dim.countSectionId || `${dim.sectionId || dimensionHeader}_count`,
  ...
  if (topBottom && metricHeader) {
    const dimensionHeader =
      ...
      findColumnHeader(
        table,
        (config.dimensions || []).flatMap((d) => d.hints || []),
      );
