...
const BRIDGE_VERSION = "query_candidate_planner_real_shadow_capture_bridge_v1";
...
function sanitizeShadowResolutionForEvidence(resolution = {}) {
  const invocation =
    ...
    {};
  const usage =
    invocation.usage || resolution.usage || resolution.tokenUsage || {};
  const cache =
  ...
  return Object.freeze({
    ...
    plannerResolution: Object.freeze({
      ...
      invocation: Object.freeze({
        ...
        providerCallCount: number(
          first(invocation.providerCallCount, resolution.providerCallCount),
        ),
        modelId: text(
          first(invocation.modelId, invocation.model, resolution.modelId),
          120,
        ),
        inputTokens: Math.trunc(
          number(
            first(
              usage.inputTokens,
              usage.promptTokens,
        ...
        observedCostMicrousd: Math.trunc(
          number(
            first(
    ...
    cache: Object.freeze({
      ...
      level: text(
        first(cache.level, cache.cacheLevel, cache.hitLevel),
        20,
      ).toUpperCase(),
...
async function runQueryCandidatePlannerApiShadowWithRealEvidenceCapture(
  args = {},
) {
    ...
    captures.set(
      ...
      Object.freeze({
        ...
        captureSha256: crypto
          .createHash("sha256")
          .update(
            JSON.stringify(sanitizeShadowResolutionForEvidence(resolution)),
          )
          .digest("hex"),
      }),
    );
